#AES by defanive
#S盒
sbox = []
#逆S盒
inv_sbox =[]
hex_symbols_to_int = {'a': 10, 'b': 11, 'c': 12, 'd': 13, 'e': 14, 'f': 15}
rcon = [[0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36],
		[0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
		[0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
		[0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]]
#GF(2^8)乘法运算
def multiply(x,y):
	res = 0
	if(y & 0x01):
		res = x
	for i in range(1,8):
		if(y & (0x01 << i)):
			temp = x
			for j in range(i):
				temp = xtime(temp)
			res = res ^ temp
	return res%0x11b
#GF(2^8)乘法运算中的xtime运算
def xtime(x):
	res = ((x<<1) ^ (((x>>7) & 1) * 0x1b))
	return res%0x100
def encrypt(input_bytes, key):
	state = [[] for j in range(4)]
	for r in range(4):
		for c in range(4):
			state[r].append(input_bytes[r + 4 * c])
	key_schedule = key_expansion(key)
	state = add_round_key(state, key_schedule)
	for rnd in range(1, 10):
		state = sub_bytes(state)
		state = shift_rows(state)
		state = mix_columns(state)
		state = add_round_key(state, key_schedule, rnd)
	state = sub_bytes(state)
	state = shift_rows(state)
	state = add_round_key(state, key_schedule, rnd + 1)
	output = [None for i in range(4 * 4)]
	for r in range(4):
		for c in range(4):
			output[r + 4 * c] = state[r][c]
	return output
#加密模块
def decrypt(cipher, key):
	state = [[] for i in range(4)]
	for r in range(4):
		for c in range(4):
			state[r].append(cipher[r + 4 * c])
	key_schedule = key_expansion(key)
	state = add_round_key(state, key_schedule, 10)
	rnd = 10 - 1
	while rnd >= 1:
		state = shift_rows(state, inv=True)
		state = sub_bytes(state, inv=True)
		state = add_round_key(state, key_schedule, rnd)
		state = mix_columns(state, inv=True)
		rnd -= 1
	state = shift_rows(state, inv=True)
	state = sub_bytes(state, inv=True)
	state = add_round_key(state, key_schedule, rnd)
	output = [None for i in range(4 * 4)]
	for r in range(4):
		for c in range(4):
			output[r + 4 * c] = state[r][c]
	return output
#字节代换
def sub_bytes(state, inv=False):
	if inv == False:  # encrypt
		box = sbox
	else:  # decrypt
		box = inv_sbox
	for i in range(len(state)):
		for j in range(len(state[i])):
			row = state[i][j] // 0x10
			col = state[i][j] % 0x10
			box_elem = box[16 * row + col]
			state[i][j] = box_elem
	return state
#行移位
def shift_rows(state, inv=False):
	count = 1
	if inv == False:  # encrypting
		for i in range(1, 4):
			state[i] = left_shift(state[i], count)
			count += 1
	else:  # decryptionting
		for i in range(1, 4):
			state[i] = right_shift(state[i], count)
			count += 1
	return state
#列混合
def mix_columns(state, inv=False):
	for i in range(4):
		if inv == False:  #加密
			s0 = xtime(state[0][i]) ^ multiply(0x3, state[1][i]) ^ state[2][i] ^ state[3][i]
			s1 = state[0][i] ^ xtime(state[1][i]) ^ multiply(0x3, state[2][i]) ^ state[3][i]
			s2 = state[0][i] ^ state[1][i] ^ xtime(state[2][i]) ^ multiply(0x3, state[3][i])
			s3 = multiply(0x3, state[0][i]) ^ state[1][i] ^ state[2][i] ^ xtime(state[3][i])
		else:  #解密
			s0 = multiply(0xe, state[0][i]) ^ multiply(0xb, state[1][i]) ^ multiply(0xd, state[2][i]) ^ multiply(0x9, state[3][i])
			s1 = multiply(0x9, state[0][i]) ^ multiply(0xe, state[1][i]) ^ multiply(0xb, state[2][i]) ^ multiply(0xd, state[3][i])
			s2 = multiply(0xd, state[0][i]) ^ multiply(0x9, state[1][i]) ^ multiply(0xe, state[2][i]) ^ multiply(0xb, state[3][i])
			s3 = multiply(0xb, state[0][i]) ^ multiply(0xd, state[1][i]) ^ multiply(0x9, state[2][i]) ^ multiply(0xe, state[3][i])
		state[0][i] = s0
		state[1][i] = s1
		state[2][i] = s2
		state[3][i] = s3
	return state
#密钥拓展
def key_expansion(key):
	key_symbols = [symbol for symbol in key]
	key_schedule = [[] for i in range(4)]
	for r in range(4):
		for c in range(4):
			key_schedule[r].append(key_symbols[r + 4 * c])
	for col in range(4, 4 * (10 + 1)):  # col - column number
		if col % 4 == 0:
# take shifted (col - 1)th column...
			tmp = [key_schedule[row][col - 1] for row in range(1, 4)]
			tmp.append(key_schedule[0][col - 1])
# change its elements using Sbox-table like in SubBytes...
			for j in range(len(tmp)):
				sbox_row = tmp[j] // 0x10
				sbox_col = tmp[j] % 0x10
				sbox_elem = sbox[16 * sbox_row + sbox_col]
				tmp[j] = sbox_elem
# and finally make XOR of 3 columns
			for row in range(4):
				s = (key_schedule[row][col - 4]) ^ (tmp[row]) ^ (rcon[row][int(col / 4 - 1)])
				key_schedule[row].append(s)
		else:
# just make XOR of 2 columns
			for row in range(4):
				s = key_schedule[row][col - 4] ^ key_schedule[row][col - 1]
				key_schedule[row].append(s)
	return key_schedule
#轮密钥加
def add_round_key(state, key_schedule, round=0):
	for col in range(4):
		s0 = state[0][col] ^ key_schedule[0][4 * round + col]
		s1 = state[1][col] ^ key_schedule[1][4 * round + col]
		s2 = state[2][col] ^ key_schedule[2][4 * round + col]
		s3 = state[3][col] ^ key_schedule[3][4 * round + col]
		state[0][col] = s0
		state[1][col] = s1
		state[2][col] = s2
		state[3][col] = s3
	return state
#左移位
def left_shift(array, count):
	res = array[:]
	for i in range(count):
		temp = res[1:]
		temp.append(res[0])
		res[:] = temp[:]
	return res
#右移位
def right_shift(array, count):
	res = array[:]
	for i in range(count):
		tmp = res[:-1]
		tmp.insert(0, res[-1])
		res[:] = tmp[:]
	return res
#字符转十六进制数组
def byte2hex(s):
	res=[0x0 for i in range(16)]
	for i in range(16):
		temp = int(ord(s[i]))
		res[i] = temp
	return res
#输出密文
def print_cipher(s):
	res=''
	for i in range(16):
		temp = hex(s[i]).replace('0x', '')
		while len(temp) < 2:
			temp = '0' + temp
		res += temp
	print('加密完成-密文: ',res)
#输出明文
def print_message(s):
	res=''
	for i in range(16):
		temp = chr(s[i])
		res += temp
	print('解密完成-明文: ',res)
#main函数
if __name__ == '__main__':
	while 1:
		option = int(input('请选择功能: (0)AES (1)GF(2^8)乘法 (其他数字退出）：'))
		if option == 0:
			print('AES128加解密')
			input_bytes = input('请输入16位英文字符-明文: ')
			input_bytes = byte2hex(input_bytes)
			key = input('请输入16位英文字符-密文: ')
			key = byte2hex(key)
			cipher = encrypt(input_bytes, key)
			print_cipher(cipher)
			message = decrypt(cipher, key)
			print_message(message)
		elif option == 1:
			print('GF(2^8)乘法运算  AxB')
			x = int(input('input A:'), 16)
			y = int(input('input B:'), 16)
			print('result:',hex(multiply(x,y))[2:].upper())
		else :
			print('See You ~~')
#defanive